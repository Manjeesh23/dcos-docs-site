...
title: Uninstalling
menuWeight: 40
excerpt: Uninstalling DC/OS Apache Couchbase Service
featureMaturity:
enterprise: false
---
...
For example, to uninstall the Apache Couchbase instance named `couchbase-dev`, run:

```shell
dcos package uninstall --app-id=couchbase-dev couchbase
```
...
This situation is indicated by looking at the Deploy Plan while the uninstall is proceeding. The Deploy Plan may be viewed using either of the following methods:
    ...
    1. CLI: dcos couchbase --name=couchbase plan show deploy (after running dcos package install --cli couchbase if needed)
    2. HTTP: https://yourcluster.com/service/couchbase/v1/plans/deploy

**Deploy Plan in progress**

```shell
dcos couchbase --name=couchbase plan show deploy
deploy (serial strategy) (IN_PROGRESS)
└─ node (serial strategy) (IN_PROGRESS)
   ├─ couchbase-0:[init] (COMPLETE)
   ├─ couchbase-0:[node] (COMPLETE)
   ├─ couchbase-0:[metrics] (STARTING)
   ├─ couchbase-1:[node] (PENDING)
   └─ couchbase-1:[metrics] (PENDING)   
```    
...
└─ node (serial strategy) (COMPLETE)
   ├─ couchbase-0:[init] (COMPLETE)
   ├─ couchbase-0:[node] (COMPLETE)
   ├─ couchbase-0:[metrics] (COMPLETE)
   ├─ couchbase-1:[node] (COMPLETE)
   └─ couchbase-1:[metrics] (COMPLETE)
```       
...
1. CLI: dcos couchbase --name=couchbase plan show deploy
2. HTTP: https://yourcluster.com/service/couchbase/v1/plans/deploy/forceComplete?phase=unreserve-resources&step=unreserve-<UUID>
...
│  ├─ kill-task- (COMPLETE)
│  ├─ kill-task-couchbase-0-init__4ea667b6-e784-4982-9c68-61af8732f552 (COMPLETE)
│  ├─ kill-task-couchbase-0-metrics__d438f4b5-d6d1-480c-8fd7-439943f5cba9 (COMPLETE)
│  ├─ kill-task-couchbase-0-node__3e16d999-0b72-4ca2-bee4-74c910acd8a3 (COMPLETE)
│  ├─ kill-task- (COMPLETE)
│  ├─ kill-task- (COMPLETE)
│  ├─ kill-task-couchbase-1-metrics__d0d1b9ca-e803-4392-9210-8088ee234df4 (COMPLETE)
│  ├─ kill-task-couchbase-1-node__a7704212-ce1f-4150-aa75-5f08a5fabbab (COMPLETE)
│  └─ kill-task- (COMPLETE)
...
```shell
       dcos package uninstall --app-id=/test/couchbase couchbase
```                   
