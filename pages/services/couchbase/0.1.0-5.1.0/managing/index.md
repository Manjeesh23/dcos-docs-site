...
title: Managing
menuWeight: 80
excerpt: Managing your DC/OS Couchbase configuration
featureMaturity:
enterprise: false
---
...
You can make changes to the service after it has been launched. Configuration management is handled by the scheduler process, which in turn handles Couchbase deployment itself.

After making a change, the scheduler will be restarted, and it will automatically deploy any detected changes to the service, one node at a time. For example, a given change will first be applied to `couchbase-0`, then `couchbase-1`, and so on.
...
+ The service's subcommand available and installed on your local machine.
  + You can install just the subcommand CLI by running `dcos package install --cli --yes couchbase`.
  + If you are running an older version of the subcommand CLI that doesn't have the `update` command, uninstall and reinstall your CLI.
    ```shell
    dcos package uninstall --cli couchbase
    dcos package install --cli couchbase
    ```
...
```shell
dcos couchbase describe > options.json
```

Make any configuration changes to the `options.json` file.

If you installed this service with a prior version of DC/OS, this configuration will not have been persisted by the the DC/OS package manager. You can instead use the `options.json` file that was used when [installing the service](https://github.com/rathohit311356/dcos-docs-site/blob/Couchbase-Service-Guide/pages/services/couchbase/0.1.0-2.2.1/install/install.md).
...
First, we'll fetch the default application's environment, current application's environment, and the actual couchbase that maps config values to the environment:
...
5. To identify those values that are custom, we'll get the default environment variables for this version of the service:
	...
	```
6. We'll also get the entire application couchbase:
	```shell
...
1. Use JQ and `diff` to compare the two:
	...
	```
2. Now compare these values to the values contained in the `env` section in application couchbase:
	```shell
...
To see a full listing of available options, run `dcos package describe --config couchbase` in the CLI, or browse the DC/OS couchbase Service install dialog in the DC/OS Dashboard.
...
```shell
10.0.10.3: couchbase-0
10.0.10.8: couchbase-1
10.0.10.26: couchbase-2
10.0.10.28: empty
10.0.10.84: empty
```
...
```
    2. Redeploy `_NODEPOD_-1` from the decommissioned node to somewhere within the new whitelist: `dcos couchbase pod replace _NODEPOD_-1`
...
1. Run `dcos couchbase pod replace couchbase-<NUM>`, e.g. `couchbase-2` to halt the current instance with id `<NUM>` (if still running) and launch a new instance on a different agent.

For example, let's say `couchbase-2`'s host system has died and `couchbase-2` needs to be moved.

1. NOW THAT THE NODE HAS BEEN DECOMMISSIONED, (IF NEEDED BY YOUR SERVICE) start `couchbase-2` at a new location in the cluster.
    ```shell
    dcos couchbase pod replace couchbase-2
    ```
...
```shell
dcos couchbase update force-complete service-phase service-0:[node]
```
...
```shell
dcos couchbase update force-restart
```

Or for all steps in a single phase:
```shell
dcos couchbase update force-restart service-phase
```

Or for a specific step within a specific phase:
```shell
dcos couchbase update force-restart service-phase service-0:[node]
```
